...
export class FirebaseService {
  ...
  public subscribeToUserFavorites(
    userId: string,
    ...
    return onSnapshot(query(collection(this._db, 'fav'), where('idUser', '==', userId)), async (snapshot) => {
        try {
            const favorites = snapshot.docs.map(doc => mapFunction(doc.data()));
            console.log('All Favorites:', favorites);
            subject.next(favorites);
        } catch (error) {
            console.error('Error:', error);
        }
    }, (error) => {
